(() => {
  ...
  async function run() {
    ...
    if (!data.session) { location.replace(appUrl("index.html?next=admin.html")); return; }
    const response = await fetch(`${String(config.supabaseUrl).replace(/\/$/, "")}/functions/v1/admin-overview`, { headers:{ Authorization:`Bearer ${data.session.access_token}`, apikey:config.supabaseAnonKey } });
    const payload = await response.json().catch(() => ({}));
    if (response.status === 403) { location.replace(appUrl("app.html")); return; }
  ...
  document.getElementById("adminLogout")?.addEventListener("click", async () => {
    ...
    await client.auth.signOut(); location.replace(appUrl("index.html"));
  });
