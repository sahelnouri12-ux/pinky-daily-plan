(() => {
  ...
  function authHeaders(extra = {}) {
    const headers = { ...extra, apikey: config.supabaseAnonKey || "" };
    return session?.access_token ? { ...headers, Authorization: `Bearer ${session.access_token}` } : headers;
  }
  ...
  async function functionFetch(name, options = {}) {
    const headers = authHeaders(options.headers || {});
    return fetch(functionUrl(name), { ...options, headers });
  }
  ...
  async function logout() {
    ...
    location.replace(appUrl("index.html"));
  }

  async function deleteAccount() {
    const response = await functionFetch("delete-account", {
      method: "DELETE",
  ...
  async function initialize() {
    if (!isConfigured || !window.supabase?.createClient) {
      location.replace(appUrl("index.html?setup=1"));
    ...
    if (!session) {
      ...
      location.replace(appUrl(`index.html?next=${next}`));
    ...
    client.auth.onAuthStateChange((event, nextSession) => {
      ...
      if (event === "SIGNED_OUT") location.replace(appUrl("index.html"));
    });
