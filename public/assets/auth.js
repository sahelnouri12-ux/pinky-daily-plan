(() => {
  ...
  const nextPath = requestedNext.startsWith("/") && !requestedNext.startsWith("//") ? requestedNext : "/app.html";
  ...
  const dict = {
    fa: {
      ...
      setup: "پورتال هنوز به Supabase متصل نشده است. فایل راهنمای نصب داخل پروژه را اجرا کن و متغیرهای محیطی را در Vercel وارد کن.",
    ...
    en: {
      ...
      setup: "The portal is not connected to Supabase yet. Follow the setup guide included in the project and add the environment variables in Vercel.",
  ...
  async function start() {
    ...
    if (data.session) {
      location.replace(nextPath.startsWith("/") ? nextPath : "/app.html");
      return;
    }
  ...
  forms.reset.addEventListener("submit", async event => {
    ...
    try {
      ...
      const { error } = await client.auth.resetPasswordForEmail(String(form.get("email")).trim(), { redirectTo: `${location.origin}/reset.html` });
      if (error) throw error;
